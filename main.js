...
const TANGENCY_TOLERANCE_DEGREES = (10 / 3600); // 10 seconds
...
function generateManualControls(warnings, existingOverrides = {}) {
    ...
    if (segmentsToControl.size === 0) return;

    const title = document.createElement('h3');
    title.className = "text-md font-semibold text-blue-800";
    title.textContent = "Manual Adjustments:";
...
function processSegment(segmentData, type, prevLineCourseStr, curveRLOverride, currentSegmentNumber, isFirstSegment, lastSegmentWasCurve, lastCurveRL, startCoordinates) {
    ...
    const segmentId = `override-segment-${currentSegmentNumber}`;
    const overrideElement = document.getElementById(segmentId);
    ...
    if (type.toLowerCase() === 'line') {
        ...
        let curve = {
            courseInStr: null,
            courseInBearing: null,
            courseInAzimuth: null,
            courseOutStr: null,
            courseOutBearing: null,
            courseOutAzimuth: null,
...
function azimuthToBearing(azimuth) {
    ...
    const epsilon = 1e-9;
    if (Math.abs(azimuth - 0) < epsilon || Math.abs(azimuth - 360) < epsilon) return {
        dir1: 'N',
        deg: 0,
        min: 0,
        sec: 0,
    ...
    if (Math.abs(azimuth - 90) < epsilon) return {
        dir1: 'E',
        deg: 0,
        min: 0,
        sec: 0,
    ...
    if (Math.abs(azimuth - 180) < epsilon) return {
        dir1: 'S',
        deg: 0,
        min: 0,
        sec: 0,
    ...
    if (Math.abs(azimuth - 270) < epsilon) return {
        dir1: 'W',
        deg: 0,
        min: 0,
        sec: 0,
...
async function generateAndDownloadDxf(coordinates) {
    ...
    try {
        ...
        let dxfContent = `999
Generated by Diamond Land Surveying Parser
  0
SECTION
  2
HEADER
